...
test('p-mining bootstrap network stats add 1-3 synthetic users on random 3-10 minute intervals and advance power only', async () => {
  ...
  try {
    ...
    assert.equal(Number(next.body.network.todayMined || 0), Number(initial.body.network.todayMined || 0));
    assert.equal(Number(next.body.network.totalMined || 0), Number(initial.body.network.totalMined || 0));
  } finally {
