...
test('p-mining html includes host header, tab panels, and script mounts', () => {
  ...
  assert.match(html, /id="pMiningInviteSubmitButton" class="p-mining-submit-button" type="button">✅</);
...
test('p-mining mobile stats stay in a compact two-column dashboard layout', () => {
  ...
  assert.match(css, /@media \(max-width:\s*720px\)[\s\S]*?\.p-mining-rule-card\s*\{[\s\S]*?line-height:\s*1\.36;/);
});
...
test('p-mining html includes the expected mining, invite, records, and profile sections', () => {
  ...
  assert.match(html, /P is Pay，P is People，P没有实际用途，当参与的人数超过 1000 万人时，可能是一场伟大的胜利。/);
