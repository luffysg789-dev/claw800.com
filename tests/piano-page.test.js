...
test('piano css includes landscape-first keyboard layout and desktop centering', () => {
  ...
  assert.doesNotMatch(css, /\.piano-page\.is-portrait\s+\.piano-stage\s*\{/);
...
test('piano script prepares note playback without forcing mobile rotation', () => {
  const html = fs.readFileSync(htmlPath, 'utf8');
  const css = fs.readFileSync(cssPath, 'utf8');
  const js = fs.readFileSync(jsPath, 'utf8');
  ...
  assert.doesNotMatch(js, /function syncOrientationState\(/);
  assert.doesNotMatch(js, /matchMedia\('\(orientation: portrait\)'\)/);
