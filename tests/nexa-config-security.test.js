...
test('browser game scripts do not hardcode the Nexa API key anymore', () => {
  for (const entry of publicFiles) {
    ...
    if (/nexaauth:\/\/oauth|NEXA_PROTOCOL_AUTH_BASE|\/api\/nexa\/tip\/session/.test(entry.source)) {
...
test('admin site config exposes Nexa credential fields without leaking the secret to browser code', () => {
  assert.match(adminHtml, /name="nexaApiBaseUrl"/);
  ...
  assert.match(serverJs, /nexaApiBaseUrl/);
  assert.match(serverJs, /postConfiguredNexaJson/);
