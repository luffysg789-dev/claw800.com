...
test('predict-master page shell loads its assets and calls backend login url API', () => {
  ...
  assert.match(html, /<button id="predictMasterTradingOrdersBtn" type="button" hidden>订单<\/button>/);
  ...
  assert.match(script, /function refreshWalletBalanceInBackground/);
  assert.match(script, /function checkPendingRechargePaymentInBackground/);
  assert.match(script, /wallet-refresh-after-order/);
  assert.match(script, /wallet-refresh-initial-load/);
  assert.match(script, /pending-recharge-initial-load/);
  assert.doesNotMatch(script, /await refreshWalletBalance\(session\)/);
  ...
  assert.match(script, /refreshWalletBalanceInBackground\(currentSession, 'wallet-refresh-focus'\)/);
  assert.match(script, /document\.addEventListener\('visibilitychange'/);
  assert.match(script, /checkPendingRechargePaymentInBackground\('pending-recharge-initial-load'\)/);
