...
function simulateXiangqiMoveState(state, piece, from, to) {
...
function hasAnyLegalXiangqiMove(state, side) {
  const normalizedSide = String(side || '').toUpperCase();
  const ownPieces = state.pieces.filter((piece) => String(piece.side || '').toUpperCase() === normalizedSide);
  ...
  for (const piece of ownPieces) {
    ...
    for (let file = 0; file < 9; file += 1) {
      for (let rank = 0; rank < 10; rank += 1) {
        ...
        const nextState = simulateXiangqiMoveState(state, piece, from, to);
...
const moveXiangqiMatch = db.transaction((payload) => {
  ...
  const nextState = simulateXiangqiMoveState(state, movingPiece, from, to);
  if (isXiangqiKingInCheck(nextState, side)) return { kind: 'illegal_move' };
  ...
  const opponentSide = nextTurnSide;
  ...
  if (isXiangqiKingInCheck(nextState, opponentSide) && !hasAnyLegalXiangqiMove(nextState, opponentSide)) {
    updateXiangqiMatchFinishedSnapshotStmt.run(
      serializedState,
      timers.redTimeLeftMs,
      timers.blackTimeLeftMs,
