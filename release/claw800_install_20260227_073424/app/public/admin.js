...
function insertTextAtCursor(textarea, snippet) {
  ...
  textarea.value = `${original.slice(0, start)}${snippet}${original.slice(end)}`;
...
function applyLanguage() {
  ...
  document.getElementById('tutorialAddTitle').textContent = dict.tutorialAddTitle;
  ...
  document.getElementById('tutorialAddBtn').textContent = dict.tutorialAddBtn;
...
insertTutorialImageBtn.addEventListener('click', () => {
  ...
  reader.onload = () => {
    ...
    insertTextAtCursor(tutorialContentInput, snippet);
...
tutorialAddForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const endpointCandidates = [
      ...getApiCandidates('/api/admin/tutorials'),
      ...getApiCandidates('/api/admin/tutorial'),
      ...getApiCandidates('/api/tutorials'),
      ...getApiCandidates('/api/tutorial')
    ];
    let res = null;
    ...
    if (!res) {
      tutorialMessage.textContent = t('tutorialRouteMissing');
      tutorialMessage.className = 'message error';
      return;
    }
    let data = {};
    ...
    if (!res.ok) {
      tutorialMessage.textContent = localizeApiError(data.error || `HTTP ${res.status} ${res.statusText}`);
      tutorialMessage.className = 'message error';
      return;
    }

    tutorialMessage.textContent = t('tutorialCreated');
    tutorialMessage.className = 'message success';
...
document.getElementById('navTutorialAdd').addEventListener('click', () => setView('tutorial-add'));
