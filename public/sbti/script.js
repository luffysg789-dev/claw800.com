(function bootstrapSbti() {
  ...
  const TYPE_IMAGES = {
    IMSB: '/sbti/image/IMSB.png',
    BOSS: '/sbti/image/BOSS.png',
    MUM: '/sbti/image/MUM.png',
    FAKE: '/sbti/image/FAKE.png',
    'Dior-s': '/sbti/image/Dior-s.jpg',
    DEAD: '/sbti/image/DEAD.png',
    ZZZZ: '/sbti/image/ZZZZ.png',
    GOGO: '/sbti/image/GOGO.png',
    FUCK: '/sbti/image/FUCK.png',
    CTRL: '/sbti/image/CTRL.png',
    HHHH: '/sbti/image/HHHH.png',
    SEXY: '/sbti/image/SEXY.png',
    OJBK: '/sbti/image/OJBK.png',
    'JOKE-R': '/sbti/image/JOKE-R.jpg',
    POOR: '/sbti/image/POOR.png',
    'OH-NO': '/sbti/image/OH-NO.png',
    MONK: '/sbti/image/MONK.png',
    SHIT: '/sbti/image/SHIT.png',
    'THAN-K': '/sbti/image/THAN-K.png',
    MALO: '/sbti/image/MALO.png',
    'ATM-er': '/sbti/image/ATM-er.png',
    'THIN-K': '/sbti/image/THIN-K.png',
    SOLO: '/sbti/image/SOLO.png',
    'LOVE-R': '/sbti/image/LOVE-R.png',
    'WOC!': '/sbti/image/WOC.png',
    DRUNK: '/sbti/image/DRUNK.png',
    IMFW: '/sbti/image/IMFW.png'
  };
  ...
  const testHint = document.getElementById('sbtiTestHint');
  const submitButton = document.getElementById('sbtiSubmitButton');
  const backButton = document.getElementById('sbtiBackButton');
  const resultCn = document.getElementById('sbtiResultCn');
  const resultCode = document.getElementById('sbtiResultCode');
  const resultName = document.getElementById('sbtiResultName');
  const resultModeKicker = document.getElementById('sbtiResultModeKicker');
  const matchBadge = document.getElementById('sbtiMatchBadge');
  const resultSub = document.getElementById('sbtiResultSub');
  const resultDesc = document.getElementById('sbtiResultDesc');
  const resultGrid = document.getElementById('sbtiResultGrid');
  const posterCaption = document.getElementById('sbtiPosterCaption');
  ...
  function getQuestionMetaLabel(question) {
    return question.special ? '补充题' : '维度已隐藏';
  }

  function renderSbtiQuestions() {
    const visibleQuestions = getVisibleQuestions();
    const optionCodes = ['A', 'B', 'C', 'D', 'E'];
    questionList.innerHTML = visibleQuestions.map((question, index) => `
      <article class="sbti-question-card">
        <div class="sbti-question-card__meta">
          <div class="sbti-question-card__index">第 ${index + 1} 题</div>
          <div class="sbti-question-card__hint">${escapeHtml(getQuestionMetaLabel(question))}</div>
        </div>
        <h3 class="sbti-question-card__title">${escapeHtml(question.text)}</h3>
        <div class="sbti-option-row">
          ${question.options.map((option, optionIndex) => {
            const checked = state.answers[question.id] === option.value;
            return `
              <label class="sbti-option${checked ? ' is-selected' : ''}" data-question-option data-question-id="${question.id}" data-value="${option.value}">
                <input type="radio" name="${question.id}" value="${option.value}" ${checked ? 'checked' : ''} />
                <span class="sbti-option__radio" aria-hidden="true"></span>
                <span class="sbti-option__value">${optionCodes[optionIndex] || String(optionIndex + 1)}</span>
                <span class="sbti-option__label">${escapeHtml(option.label)}</span>
              </label>
            `;
          }).join('')}
        </div>
  ...
  function renderDimList(result) {
    resultGrid.innerHTML = dimensionOrder.map((dim) => {
      const level = result.levels[dim];
      const score = result.rawScores[dim];
      const explanation = DIM_EXPLANATIONS[dim][level];
      return `
        <div class="sbti-result-metric">
          <div class="sbti-result-metric__head">
            <span class="sbti-result-metric__axis">${escapeHtml(dimensionMeta[dim].name)}</span>
            <span class="sbti-result-metric__rating">${level} / ${score}分</span>
          </div>
          <p class="sbti-result-metric__desc">${escapeHtml(explanation)}</p>
        </div>
      `;
    }).join('');
  }
  ...
  function renderSbtiResultFromData(result) {
    ...
    resultCode.textContent = type.code;
    resultName.textContent = `${type.code}（${type.cn}）`;
    ...
    funNote.textContent = DEFAULT_FUN_NOTE;
  ...
  questionList.addEventListener('click', (event) => {
    ...
    if (questionId === 'drink_gate_q1') {
      if (value !== 3) delete state.answers.drink_gate_q2;
      renderSbtiQuestions();
      return;
    }
